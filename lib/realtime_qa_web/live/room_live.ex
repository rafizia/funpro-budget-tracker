defmodule RealtimeQaWeb.RoomLive do
        ...
        <div class="mb-8">
          ...
          <form phx-submit="add_question">
            <input
              ...
              required
            />
            <button class="bg-blue-700 text-lg font-medium text-white px-4 py-2 rounded mt-3 hover:bg-blue-900 transition">
              Submit Question
            </button>
          </form>
        </div>
  ...
  def handle_event("join_room", %{"code" => code}, socket),
    do: {:noreply, push_patch(socket, to: ~p"/room/#{code}")}
  ...
  def handle_event("edit_question", %{"id" => id}, socket),
    do: {:noreply, assign(socket, editing_question_id: String.to_integer(id))}
  ...
  defp upvote_button_class(question_id, upvoted_set) do
    if is_upvoted?(question_id, upvoted_set),
      ...
      else: "flex flex-col items-center text-gray-600 hover:text-gray-800"
  end
end
